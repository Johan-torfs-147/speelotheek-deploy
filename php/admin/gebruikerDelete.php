...
if(!isset($_SESSION['sess_user']) || ($_SESSION['sess_role'] != "Admin")){
    ...
    if ($contentType === "application/json") {
        ...
        if(is_array($decoded)) {
            ...
            $sql = mysqli_prepare($conn, "DELETE FROM vzwballonneke.reservatie WHERE gebruikerId IN ($in);");
            ...
            // Set query review
            $sql = mysqli_prepare($conn, "DELETE FROM vzwballonneke.review WHERE gebruikerId IN ($in);");
            mysqli_stmt_bind_param($sql, $types, ...$ids);
            if (!mysqli_stmt_execute($sql)) {
                $success = false;
            }
            ...
            // Set query inhoud
            $sql = mysqli_prepare($conn, "DELETE FROM vzwballonneke.gebruiker WHERE id IN ($in);");
            mysqli_stmt_bind_param($sql, $types, ...$ids);
            if (!mysqli_stmt_execute($sql)) {
                $success = false;
            }
