...
if(!isset($_SESSION['sess_user']) || $_SESSION['sess_role'] != "Admin"){
    ...
    if ($contentType === "application/json") {
        ...
        if(is_array($decoded)) {
            ...
            $ids = $decoded['ids'];
            $bevestigd = $decoded['bevestigd'] ? 1 : 0;
            ...
            $sql = mysqli_prepare($conn, "UPDATE vzwballonneke.reservatie SET bevestigd = ? WHERE id IN ($in);");
            mysqli_stmt_bind_param($sql, 'i'.$types,$bevestigd, ...$ids);
