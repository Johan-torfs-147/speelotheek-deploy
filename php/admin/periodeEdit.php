...
if(!isset($_SESSION['sess_user']) || $_SESSION['sess_role'] != "Admin"){
    ...
    if ($contentType === "application/json") {
        ...
        if(is_array($decoded)) {
            ...
            $start = $decoded['editedPeriodeStart'];
            $eind = $decoded['editedPeriodeEind'];
            $id = $decoded['periodeId'];
            $isArchief = $decoded['isArchief'];
            ...
            // Set query
            $sql = mysqli_prepare($conn, "UPDATE vzwballonneke.periode SET startDatum = ?, eindDatum = ? WHERE id = ?;");
            mysqli_stmt_bind_param($sql, 'ssi', $start, $eind, $id);
